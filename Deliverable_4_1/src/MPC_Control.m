classdef MPC_Control
  ...
  methods
    function mpc = MPC_Control(sys, Ts, draw)
      ...
      mpc.ctrl_opt = mpc.setup_controller(draw);
    end
    ...
    function u = get_u(mpc, x, ref)
      ...
      if nargin < 3
        ...
        if (isfeasible) disp(isfeasible); end  % error msg: 1 = infeasible, 4 = numerical problem
        assert(isfeasible==0, 'infeasible in target computation');
      end
      ...
      if (isfeasible) disp(isfeasible); end  % error msg
      assert(isfeasible==0, 'infeasible in control computation');
    end
  end
